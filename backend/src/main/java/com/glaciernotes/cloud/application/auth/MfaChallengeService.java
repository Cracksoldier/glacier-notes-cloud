...
public class MfaChallengeService {
    ...
    public LoginResult.SessionIssued complete(
        ...
        var acceptedFactor = verify(enrollment, user, code, now);
        ...
        return sessionIssuer.issue(
            user, challenge.rememberMe(), now, settings, address, description
        );
    }
    ...
    private String verify(UserMfaTotpEntity enrollment, UserEntity user, String code, Instant now) {
        ...
        if (mfaRepository.consumeRecoveryCode(user.id(), mfaTokens.hashRecoveryCode(code))) {
