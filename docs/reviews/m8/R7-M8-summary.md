...
| Minor | Version-history Escape handling | **Confirmed.** `onKeydown()` closes the lightbox first but does not close the open history panel before invoking the whole-editor close flow. Escape can therefore save/snapshot and dismiss the editor when the user intended to leave history. | **Resolved in Batch 3.** Escape uses layered dismissal and invalidates stale history requests; see the [remediation evidence](../remediation/batch-3/B3-FRONTEND-ASYNC-summary.md). |
| Major | Editor close after snapshot failure | **Confirmed.** Note persistence completes before the secondary history snapshot. If that request fails, the shared `try` skips `closeEditor()` and `refresh()`, leaving an already-saved editor blocked open and making close dependent on history availability. Report snapshot failure independently and always complete the close flow. | **Resolved in Batch 3.** Snapshot failure is isolated from editor close and refresh; see the [remediation evidence](../remediation/batch-3/B3-FRONTEND-ASYNC-summary.md). |
